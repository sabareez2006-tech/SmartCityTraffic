...
Fig.~\ref{fig:graph02} compares the Mean Absolute Error across six traffic prediction methods. The proposed FL-LSTM achieves MAE $= 7.05$, which represents a 61.7\% improvement over ARIMA (MAE $= 18.42$) and a 52.6\% improvement over SVR (MAE $= 14.87$). The improvement factor over ARIMA is quantified as:

\begin{equation}
\text{IF}_{\text{ARIMA}} = \frac{\text{MAE}_{\text{ARIMA}} - \text{MAE}_{\text{proposed}}}{\text{MAE}_{\text{ARIMA}}} \times 100 = 61.7\%
\end{equation}

\noindent Compared to the centralized LSTM (MAE $= 6.53$), the proposed federated approach incurs a modest 7.96\% accuracy penalty---the \textit{privacy cost}---which represents the information loss from not accessing raw cross-zone data. FedAvg without compression (MAE $= 7.28$) and FedProx (MAE $= 7.61$) exhibit 3.26\% and 7.94\% higher error respectively, indicating that our compression mechanism paradoxically acts as an implicit regularizer, preventing overfitting to noisy gradients.
...
Fig.~\ref{fig:graph07} presents a comprehensive comparison of four dispatch strategies across service rate and fleet utilization. The proposed dynamic FL-based dispatch achieves a service rate of 60.2\% and fleet utilization of 56.8\%, outperforming all baselines. The relative improvement over the static nearest-first strategy is:

\begin{equation}
\Delta_{\text{SR}} = \frac{60.2 - 46.6}{46.6} \times 100 = 29.2\%
\end{equation}

\noindent The zone-balanced strategy (SR $= 52.8\%$) distributes vehicles uniformly but ignores temporal demand dynamics, while random dispatch (SR $= 31.4\%$) serves as a lower bound. The 29.2\% improvement of dynamic over static is attributable to the prediction-driven repositioning that anticipates demand surges with lead time $\Delta t_{\text{lead}} = 15$ minutes (one time step), enabling preemptive vehicle relocation to high-demand zones before demand materializes.
